...
const getValidOptions = (el, menuIndex) => getMenus(el)[menuIndex].querySelectorAll('.dy-cascader-node[tabindex="-1"]');
const getLabel = el => el.querySelector('.dy-cascader-node__label').textContent;
...
describe('CascaderPanel', () => {
  ...
  it('multiple mode', async () => {
    ...
    const el = vm.$el;
    const checkbox = getOptions(el, 0)[0].querySelector('.dy-checkbox');
    expect(checkbox).to.exist;
    expect(checkbox.querySelector('.dy-checkbox__input').className).to.not.includes('is-checked');
    checkbox.querySelector('input').click();

    await waitImmediate();
    expect(checkbox.querySelector('.dy-checkbox__input').className).to.includes('is-checked');
  ...
  it('multiple mode with disabled default value', async () => {
    ...
    await waitImmediate();
    expect(checkbox).to.exist;
    expect(checkbox.querySelector('.dy-checkbox__input').className).to.includes('is-indeterminate');
    checkbox.querySelector('input').click();
  ...
  it('check strictly in single mode', async () => {
    ...
    const el = vm.$el;
    const radio = getOptions(el, 0)[0].querySelector('.dy-radio');
  ...
  it('check strictly in multiple mode', async () => {
    ...
    getOptions(el, 0)[0].click();
    await waitImmediate();
    expect(getOptions(el, 1)[0].querySelector('.dy-checkbox').className).to.not.includes('is-checked');
    getOptions(el, 1)[0].querySelector('input').click();
    await waitImmediate();
    expect(vm.value).to.deep.equal(['zhejiang', 'hangzhou']);
  ...
  it('dynamic loading', async () => {
    ...
    firstOption.click();
    await waitImmediate();
    expect(firstOption.querySelector('i').className).to.includes('dy-icon-loading');
    await wait(1000);
    expect(firstOption.querySelector('i').className).to.includes('dy-icon-arrow-right');
    expect(getMenus(el).length).to.equal(2);
    getOptions(el, 1)[0].click();
    await wait(1000);
    getOptions(el, 2)[0].click();
