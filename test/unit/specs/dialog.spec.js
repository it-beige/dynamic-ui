...
describe('Dialog', () => {
  ...
  it('create', done => {
    vm = createVue({
      template: `
        <div>
          <dy-dialog :title="title" :visible="visible"></dy-dialog>
        </div>
      `,
    ...
    setTimeout(() => {
      ...
      expect(vm.$el.querySelector('.dy-dialog__title').textContent).to.equal('dialog test');
  ...
  it('render correct content', done => {
    vm = createVue({
      template: `
        <div>
          <dy-dialog :title="title" :visible="visible">
            <span>这是一段信息</span>
            <span slot="footer" class="dialog-footer">
              <dy-button @click.native="dialogVisible = false">取消</dy-button>
              <dy-button type="primary" @click.native="dialogVisible = false">确定</dy-button>
            </span>
          </dy-dialog>
        </div>
      `,
    ...
    setTimeout(() => {
      const footerBtns = vm.$el.querySelectorAll('.dy-dialog__footer .dy-button');
      expect(vm.$el.querySelector('.dy-dialog__body span').textContent).to.equal('这是一段信息');
  ...
  it('append to body', done => {
    vm = createVue({
      template: `
        <div>
          <dy-dialog :title="title" append-to-body :visible="visible"></dy-dialog>
        </div>
      `,
  ...
  it('open and close', done => {
    vm = createVue({
      template: `
        <div>
          <dy-dialog :title="title" :visible.sync="visible">
            <span>这是一段信息</span>
          </dy-dialog>
        </div>
      `,
  ...
  describe('props', () => {
    const getDialogVm = (props, options) => {
      return createVue(Object.assign({
        template: `
          <div>
            <dy-dialog ${ props } :title="title" :visible="visible">
              <span>这是一段信息</span>
            </dy-dialog>
          </div>
        `,
    ...
    it('fullscreen', () => {
      ...
      const dialogEl = vm.$el.querySelector('.dy-dialog');
    ...
    it('top', () => {
      ...
      expect(vm.$el.querySelector('.dy-dialog').style.marginTop).to.equal('100px');
    });

    it('custom-class', () => {
      vm = getDialogVm('custom-class="my-dialog"');
      expect(vm.$el.querySelector('.dy-dialog').classList.contains('my-dialog')).to.true;
    });
  });
  ...
  it('click header btn', done => {
    ...
    const dialog = vm.$children[0];
    setTimeout(() => {
      dialog.$el.querySelector('.dy-dialog__headerbtn').click();
      setTimeout(() => {
        expect(vm.visible).to.be.false;
        done();
      }, 500);
  ...
  it('before close', done => {
    ...
    vm = createVue({
      template: `
        <div>
          <dy-dialog :title="title" :visible="visible" :before-close="beforeClose"></dy-dialog>
        </div>
      `,
  ...
  it('destroyOnClose', async () => {
    vm = createVue({
      template: `
        <div>
          <dy-dialog :title="title" :visible.sync="visible" destroy-on-close>
            <input />
          </dy-dialog>
        </div>
      `,
