...
describe('Descriptions', () => {
  ...
  it('render test', () => {
    vm = createVue(
      {
        template: `
          <dy-descriptions title="title" extra="extra">
            <dy-descriptions-item v-for="item in 4" :key="item"></dy-descriptions-item>
          </dy-descriptions>
        `
      },
      true
    ...
    const el = vm.$el;
    expect(el.querySelector('.dy-descriptions__title').textContent).to.equal('title');
    expect(el.querySelector('.dy-descriptions__extra').textContent).to.equal('extra');
    expect(el.querySelectorAll('.dy-descriptions-item__label').length).to.equal(4);
  });
  ...
  it('should render border props', () => {
    vm = createVue(
      {
        template: `
          <dy-descriptions border>
            <dy-descriptions-item v-for="item in 3" :label="item" :key="item">{{ item }}</dy-descriptions-item>
          </dy-descriptions>
        `
      },
      true
  ...
  it('should render class props', () => {
    vm = createVue(
      {
        template: `
          <dy-descriptions border label-class-name="label-class-name" content-class-name="content-class-name">
            <dy-descriptions-item v-for="item in 3" :label="item" :key="item">{{ item }}</dy-descriptions-item>
          </dy-descriptions>
        `
      },
    ...
    expect(Array.from(vm.$el.querySelector('.dy-descriptions-item__label').classList)).to.contain('label-class-name');
  ...
  it('should render direction props', async () => {
    vm = createVue({
      ...
      template: `
      <dy-descriptions :column="5" :direction="direction" border>
        <dy-descriptions-item v-for="item in 10" :label="item" :key="item">{{ item }}</dy-descriptions-item>
      </dy-descriptions>
    `,
      data () {
    ...
    vm = createVue({
      template: `
        <dy-descriptions :column="3">
          <dy-descriptions-item label="1">1</dy-descriptions-item>
          <dy-descriptions-item label="2" :span="2">2</dy-descriptions-item>
          <dy-descriptions-item label="3">3</dy-descriptions-item>
        </dy-descriptions>
      `
    }, true);
  ...
  it('re-rendered when slots is updated', async () => {
    ...
    vm = createVue({
      template: `
        <div>
          <dy-descriptions v-for="(remark,index) in remarks" :key="index" :title="remark">
            <dy-descriptions-item label="remark">
              <dy-tag size="small">{{remark}}</dy-tag>
    ...
    await waitImmediate();
    expect(vm.$el.querySelector('.dy-tag').innerText).to.equal(CHANGE_VALUE);
  });
});
