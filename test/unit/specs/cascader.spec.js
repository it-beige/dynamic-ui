...
const getMenus = el => el.querySelectorAll('.dy-cascader-menu');
const getOptions = (el, menuIndex) => getMenus(el)[menuIndex].querySelectorAll('.dy-cascader-node');
const selectedValue = ['zhejiang', 'hangzhou', 'xihu'];
const getCloseButton = el => el.querySelectorAll('i.dy-tag__close');
...
describe('Cascader', () => {
  ...
  it('expand and check', async() => {
    ...
    expect(checkedOption.querySelector('i.dy-icon-check')).to.exist;
  ...
  it('with default value', async() => {
    ...
    expect(getOptions(el, 2)[0].querySelector('i').className).to.includes('dy-icon-check');
    expect(vm.$el.querySelector('input').value).to.equal('Zhejiang / Hangzhou / West Lake');
  });
  ...
  it('clearable', async() => {
    ...
    await waitImmediate();
    const closeBtn = vm.$el.querySelector('i.dy-input__icon');
  ...
  it('multiple mode', async() => {
    ...
    getOptions(document.body, 0)[0].querySelector('.dy-checkbox input').click();
    await waitImmediate();
    expect(vm.value.length).to.equal(3);
    expect(getCloseButton(vm.$el).length).to.equal(3);

    const tags = vm.$el.querySelectorAll('.dy-tag');
    const closeBtn = tags[0].querySelector('.dy-tag__close');
    expect(tags.length).to.equal(3);
    expect(closeBtn).to.exist;
    closeBtn.click();
    await waitImmediate();
    expect(vm.value.length).to.equal(2);
    expect(vm.$el.querySelectorAll('.dy-tag').length).to.equal(2);
  ...
  it('clearable in multiple mode', async() => {
    ...
    await waitImmediate();
    expect(getOptions(document.body, 0)[0].querySelector('.dy-checkbox.is-checked')).to.exist;
    triggerEvent(vm.$el, 'mouseenter');
    await waitImmediate();
    const closeBtn = vm.$el.querySelector('i.dy-input__icon');
    expect(closeBtn).to.exist;
  ...
  it('collapse tags', async() => {
    ...
    expect(tags[0].querySelector('.dy-tag__close')).to.exist;
    expect(tags[1].querySelector('.dy-tag__close')).to.be.null;
    tags[0].querySelector('.dy-tag__close').click();
    expect(tags[1].textContent).to.equal('+ 3');
    await waitImmediate();
    expect(vm.value.length).to.equal(3);
    vm.$el.querySelector('.dy-tag .dy-tag__close').click();
    await waitImmediate();
    vm.$el.querySelector('.dy-tag .dy-tag__close').click();
    await waitImmediate();
    expect(vm.$el.querySelector('.dy-tag')).to.exist;
  ...
  it('filterable', async() => {
    ...
    await wait(300);
    expect(body.querySelector('.dy-cascader__suggestion-list')).to.exist;
    expect(body.querySelectorAll('.dy-cascader__suggestion-item').length).to.equal(3);
    body.querySelectorAll('.dy-cascader__suggestion-item')[0].click();
    await waitImmediate();
    expect(vm.value).to.deep.equal(selectedValue);
  });
  ...
  it('filter method', async() => {
    ...
    triggerEvent(input, 'input');
    await wait(300);
    expect(body.querySelector('.dy-cascader__suggestion-item').textContent).to.equal('Zhejiang / Hangzhou / West Lake');
  });
});
