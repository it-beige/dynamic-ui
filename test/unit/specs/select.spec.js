...
describe('Select', () => {
  ...
  it('create', () => {
    ...
    expect(vm.$el.querySelector('.dy-input__inner').placeholder).to.equal('请选择');
    vm.toggleMenu();
  ...
  it('options rendered correctly', () => {
    vm = getSelectVm();
    const options = vm.$el.querySelectorAll('.dy-select-dropdown__item');
  ...
  it('custom dropdown class', () => {
    ...
    const dropdown = vm.$el.querySelector('.dy-select-dropdown');
  ...
  it('default value', done => {
    ...
    setTimeout(() => {
      expect(vm.$el.querySelector('.dy-input__inner').value).to.equal('双皮奶');
      done();
    }, 100);
  });
  ...
  it('single select', done => {
    vm = createVue({
      template: `
        <div>
          <dy-select v-model="value" @change="handleChange">
            <dy-option
              v-for="item in options"
              :label="item.label"
              :key="item.value"
  ...
  it('disabled select', () => {
    ...
    expect(vm.$el.querySelector('.dy-input').classList.contains('is-disabled')).to.true;
  });
  ...
  it('visible event', done => {
    vm = createVue({
      template: `
        <div>
          <dy-select v-model="value" @visible-change="handleVisibleChange">
            <dy-option
              v-for="item in options"
              :label="item.label"
              :key="item.value"
  ...
  it('clearable', done => {
    ...
    setTimeout(() => {
      const iconClear = vm.$el.querySelector('.dy-input__icon.dy-icon-circle-close');
  ...
  it('object typed value', done => {
    vm = createVue({
      template: `
        <div>
          <dy-select v-model="value" value-key="id">
            <dy-option
              v-for="item in options"
              :label="item.label"
              :key="item.id"
    ...
    setTimeout(() => {
      expect(vm.$el.querySelector('.dy-input__inner').value).to.equal('label1');
      expect(vm.$el.querySelector('.dy-select-dropdown__item').classList.contains('selected'));
      done();
    }, 100);
  });
  ...
  it('prefixed icon', () => {
      ...
      template: `
        <div>
          <dy-select v-model="value">
            ...
            <i slot="prefix" class="el-input__icon dy-icon-search"></i>
          </dy-select>
        </div>
      `,
    ...
    expect(vm.$el.querySelector('.dy-input__icon').classList.contains('dy-icon-search')).to.be.true;
  });
  ...
  it('custom el-option template', () => {
    ...
    expect(vm.$el.querySelector('.dy-select-dropdown__item p').textContent).to.equal('label value');
  });
  ...
  it('option group', () => {
    ...
    const groups = vm.$el.querySelectorAll('.dy-select-group__wrap');
    const options = groups[1].querySelectorAll('.dy-select-dropdown__item');
  ...
  it('allow create', done => {
    ...
    setTimeout(() => {
      ...
      select.onInputChange();
      setTimeout(() => {
        const options = document.querySelectorAll('.dy-select-dropdown__item span');
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          const tagCloseIcons = vm.$el.querySelectorAll('.dy-tag__close');
  ...
  it('multiple remove-tag', done => {
    ...
    vm = createVue({
      template: `
        <div>
          <dy-select v-model="value" multiple @remove-tag="handleRemoveTag">
            <dy-option
              v-for="item in options"
              :label="item.label"
              :key="item.value"
  ...
  it('event:focus & blur', done => {
    vm = createVue({
      template: `
        <dy-select ref="select"></dy-select>
      `
    }, true);
  ...
  it('should return focus to input inside select after option select', done => {
    vm = createVue({
      template: `
        <div>
          <dy-select v-model="value" ref="select">
            <dy-option label="1" :value="1" />
          </dy-select>
        </div>
      `,
      data() {
    ...
    const option = vm.$el.querySelectorAll('.dy-select-dropdown__item')[0];
  ...
  it('only emit change on user input', done => {
    ...
    vm = createVue({
      template: `
        <div>
          <dy-select v-model="value" @change="change" ref="select">
            <dy-option label="1" :value="1" />
            <dy-option label="2" :value="2" />
            <dy-option label="3" :value="3" />
          </dy-select>
        </div>
      `,
      data() {
  ...
  it('should set placeholder to label of selected option when filterable is true and multiple is false', async() => {
    vm = createVue({
      template: `
        <div>
          <dy-select ref="select" v-model="value" filterable>
            <dy-option label="test" value="test" />
          </dy-select>
        </div>
      `,
      data() {
    ...
    expect(vm.$el.querySelector('.dy-input__inner').placeholder).to.be.equal('test');
  ...
  it('default value is null or undefined', async() => {
    ...
    vm.value = null;
    await waitImmediate();
    expect(vm.$el.querySelector('.dy-input__inner').value).to.equal('');
    vm.value = '选项1';
    await waitImmediate();
    expect(vm.$el.querySelector('.dy-input__inner').value).to.equal('黄金糕');
  });
