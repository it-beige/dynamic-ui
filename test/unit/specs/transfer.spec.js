...
describe('Transfer', () => {
  ...
  const createTransfer = (props, opts) => {
    return createVue(Object.assign({
      template: `
        <dy-transfer :data="testData" ref="transfer" ${props}>
        </dy-transfer>
      `,
  ...
  it('filterable', done => {
    ...
    const leftList = transfer.$el.querySelector('.dy-transfer-panel').__vue__;
    leftList.query = '1';
    setTimeout(_ => {
  ...
  it('customize', () => {
    ...
    const label = transfer.querySelector('.dy-transfer-panel__header .dy-checkbox__label');
    expect(label.innerText.indexOf('表1') > -1).to.true;
    expect(transfer.querySelector('.dy-transfer-panel__list .dy-checkbox__label span').innerText).to.equal('1 - 备选项 1');
  ...
  it('check', () => {
    ...
    const leftList = vm.$refs.transfer.$el.querySelector('.dy-transfer-panel').__vue__;
  ...
  describe('target order', () => {
    it('original(default)', done => {
      ...
      setTimeout(() => {
        transfer.addToRight();
        setTimeout(() => {
          const targetItems = [].slice.call(vm.$el.querySelectorAll('.dy-transfer__buttons + .dy-transfer-panel .dy-transfer-panel__body .dy-checkbox__label span'));
