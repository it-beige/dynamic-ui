...
describe('Drawer', () => {
  ...
  it('create', async () => {
    ...
    expect(vm.$el.querySelector('.dy-drawer__header').textContent).to.equal(
      title
    );
  ...
  it('render correct content', async () => {
    vm = createVue(
      {
        template: `
          <dy-drawer :title='title' :visible='visible'>
            <span>这是一段信息</span>
    ...
    await waitImmediate();
    expect(vm.$el.querySelector('.dy-drawer__body span').textContent).to.equal(
      '这是一段信息'
    );
    const footerBtns = vm.$el.querySelectorAll('.dy-button');
  ...
  it('should append to body, when append-to-body flag is true', async () => {
    vm = createVue(
      {
        template: `
        <dy-drawer :title='title' :visible='visible' :append-to-body='true'>
           <span> content </span>
        </dy-drawer>
      `,
        data () {
  ...
  it('should close dialog by clicking the close button', async () => {
    ...
    await waitImmediate();
    vm.$children[0].$el.querySelector('.dy-drawer__close-btn').click();
  ...
  it('should have custom classes when custom classes were given', async () => {
    ...
    vm = createVue({
      template: `
        <dy-drawer :title='title' :visible='visible' ref='drawer' custom-class='${classes}'>
           <span>${content}</span>
        </dy-drawer>
      `,
      data () {
  ...
  it('should not render header when withHeader attribute is false', () => {
    vm = createVue({
      template: `
        <dy-drawer :title='title' :visible='visible' ref='drawer' :with-header='false'>
           <span>${content}</span>
        </dy-drawer>
      `,
      data () {
    ...
    expect(vm.$el.querySelector('.dy-drawer__header')).to.not.exist;
  });
  ...
  describe('directions', () => {
    const renderer = direction => {
      return createVue({
        template: `
         <dy-drawer :title='title' :visible='visible' direction='${direction}'>
            <span>${content}</span>
         </dy-drawer>
        `,
        data: {
  ...
  describe('size', () => {
    const renderer = (size, isVertical) =>
      createVue({
        template: `
         <dy-drawer :title='title' :visible='visible' direction='${isVertical ? 'ltr' : 'ttb'}' size='${size}'>
            <span>${content}</span>
         </dy-drawer>
      `,
        data: {
