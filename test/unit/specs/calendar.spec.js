...
describe('Calendar', () => {
  ...
  it('create', async () => {
    ...
    const titleEl = vm.$el.querySelector('.dy-calendar__title');
    expect(/2019.*4/.test(titleEl.innerText)).to.be.true;
    expect(vm.$el.querySelectorAll('thead th').length).to.equal(7);
    const rows = vm.$el.querySelectorAll('.dy-calendar-table__row');
  ...
  it('range tow monthes', async () => {
    ...
    const titleEl = vm.$el.querySelector('.dy-calendar__title');
    expect(/2019.*4/.test(titleEl.innerText)).to.be.true;
    const dateTables = vm.$el.querySelectorAll('.dy-calendar-table.is-range');
  ...
  it('firstDayOfWeek', async () => {
    ...
    const head = vm.$el.querySelector('.dy-calendar-table thead');
    expect(head.firstElementChild.innerText).to.be.equal('日');
    expect(head.lastElementChild.innerText).to.be.equal('六');
    const firstRow = vm.$el.querySelector('.dy-calendar-table__row');
