...
describe('Image', () => {
  ...
  it('create', async () => {
    ...
    const placeholder = vm.$el.querySelector('.dy-image__placeholder');
    const error = vm.$el.querySelector('.dy-image__error');
    let img = vm.$el.querySelector('.dy-image__inner');
    expect(placeholder).to.exist;
    expect(error).to.be.null;
    expect(img).to.be.null;

    await wait();
    img = vm.$el.querySelector('.dy-image__inner');
  ...
  it('$attrs', async () => {
    ...
    await wait();
    const $img = vm.$el.querySelector('.dy-image__inner');
  ...
  it('pass event listeners', async () => {
    ...
    await wait();
    vm.$el.querySelector('.dy-image__inner').click();
    await wait();
  ...
  it('big image preview', async () => {
    ...
    expect(document.querySelector('.dy-image-viewer__wrapper')).to.exist;
    document.querySelector('.dy-image-viewer__close').click();
    await wait(1000);
    expect(document.querySelector('.dy-image-viewer__wrapper')).to.not.exist;
  });
});
