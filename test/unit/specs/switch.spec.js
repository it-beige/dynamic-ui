...
describe('Switch', () => {
  ...
  it('create', () => {
    ...
    const core = vm.$el.querySelector('.dy-switch__core');
    expect(core.style.backgroundColor).to.equal('rgb(255, 0, 0)');
    expect(core.style.width).to.equal('100px');
    expect(vm.$el.querySelector('.dy-switch__label--left').querySelector('span').textContent).to.equal('off');
  });

  it('switch with icons', () => {
    vm = createTest(Switch, {
      activeIconClass: 'dy-icon-check',
      inactiveIconClass: 'dy-icon-close'
    });

    const icon = vm.$el.querySelector('.dy-switch__label--left').querySelector('i');
  ...
  it('disabled switch should not respond to user click', done => {
    vm = createVue({
      template: `
        <div>
          <dy-switch disabled v-model="value"></dy-switch>
        </div>
      `,
    ...
    vm.$el.querySelector('.dy-switch__core').click();
    Vue.nextTick(() => {
  ...
  it('value is the single source of truth', done => {
    vm = createVue({
      template: `
        <div>
          <dy-switch :value="true"></dy-switch>
        </div>
      `
    }, true);
