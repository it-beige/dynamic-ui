...
describe('Avatar', () => {
  ...
  it('shape', () => {
    vm = createVue({
      template: `
        <dy-avatar size="small" shape="square">
          user
        </dy-avatar>
      `
    }, true);
  ...
  it('icon avatar', () => {
    vm = createVue({
      template: `
        <dy-avatar icon="dy-icon-user-solid">
        </dy-avatar>
      `
    }, true);
    const avatarElm = vm.$el;
    ...
    expect(iconELm.classList.contains('dy-icon-user-solid')).to.be.true;
  });

  it('image avatar', () => {
    vm = createVue({
      template: `
        <dy-avatar src="${IMAGE_SUCCESS}"></dy-avatar>
      `
    }, true);
  ...
  it('image fit', async() => {
    vm = createVue({
      template: `
        <div>
          <dy-avatar :src="url"></dy-avatar>
          <dy-avatar :src="url" v-for="fit in fits" :fit="fit" :key="fit"></dy-avatar>
        </div>
