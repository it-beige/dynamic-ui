...
describe('DatePicker', () => {
  ...
  it('select date', done => {
    ...
    setTimeout(_ => {
      ...
      const spans = $el.querySelectorAll('.dy-date-picker__header-label');
      const arrowLeftElm = $el.querySelector('.dy-date-picker__prev-btn.dy-icon-arrow-left');
      const arrowRightElm = $el.querySelector('.dy-date-picker__next-btn.dy-icon-arrow-right');

      expect(spans[0].textContent).to.include(date.getFullYear());
      expect(spans[1].textContent).to.include(date.getMonth() + 1);
      $el.querySelector('.dy-date-picker__prev-btn.dy-icon-d-arrow-left').click();
      let count = 20;
      while (--count) {
        arrowLeftElm.click();
      }
  ...
  it('disabled clear value', done => {
    ...
    setTimeout(_ => {
      ...
      $el.querySelector('td.available').click();
      vm.$nextTick(_ => {
        vm.$el.querySelector('.dy-input__icon').click();
        setTimeout(_ => {
  ...
  it('reset', done => {
    ...
    setTimeout(_ => {
      const $el = vm.$refs.compo.picker.$el;
      $el.querySelector('.dy-date-picker__next-btn.dy-icon-arrow-right').click();
      setTimeout(_ => {
        $el.querySelector('td.available').click();
        vm.$nextTick(_ => {
          vm.value = '';
  ...
  it('focus', done => {
    vm = createVue({
      template: `
        <dy-date-picker ref="picker"></dy-date-picker>
      `
    }, true);
  ...
  describe('value-format', () => {
    ...
    it('emits', done => {
      ...
      setTimeout(_ => {
        vm.$refs.compo.picker.$el.querySelector('.dy-date-table td.available').click();
        setTimeout(_ => {
    ...
    it('accepts', done => {
      ...
      setTimeout(_ => {
        ...
        vm.$refs.compo.picker.$el.querySelector('.dy-date-table .current').click();
        setTimeout(_ => {
          expect(input.value).to.equal('2000-02-01');
          done();
        }, DELAY);
  ...
  describe('default value', () => {
    ...
    it('it works', done => {
      ...
      setTimeout(_ => {
        const $el = vm.$refs.compo.picker.$el;
        expect($el.querySelector('.dy-month-table td.default')).to.exist;
        expect($el.querySelector('.dy-year-table td.default')).to.exist;
        const defaultEls = $el.querySelectorAll('.dy-date-table td.default');
    ...
    it('is reactive, works with clear', done => {
      ...
      setTimeout(_ => {
        ...
        expect($el.querySelector('.dy-date-table td.current')).to.exist;
        vm.defaultValue = defaultValue;
        triggerEvent(vm.$refs.compo.$el, 'mouseenter');
        setTimeout(_ => {
          vm.$el.querySelector('.dy-icon-circle-close').click();
          setTimeout(_ => {
            input.focus();
            setTimeout(() => {
              $el = vm.$refs.compo.picker.$el;
              expect($el.querySelector('.dy-date-table td.current')).to.not.exist;
              expect($el.querySelector('.dy-date-table td.default')).to.exist;
              $el.querySelector('.dy-date-table td.default').click();
              setTimeout(() => {
                expect(vm.value).to.eql(expectValue);
                done();
              }, DELAY);
  ...
  describe('navigation', () => {
    ...
    const initNavigationTest = async(value) => {
      ...
      await wait();
      const $el = vm.$refs.compo.picker.$el;
      prevMonth = $el.querySelector('button.dy-icon-arrow-left');
      prevYear = $el.querySelector('button.dy-icon-d-arrow-left');
      nextMonth = $el.querySelector('button.dy-icon-arrow-right');
      nextYear = $el.querySelector('button.dy-icon-d-arrow-right');
      getYearLabel = () => $el.querySelectorAll('.dy-date-picker__header-label')[0].textContent;
    ...
    it('month label with fewer dates', async() => {
      ...
      const juneLabel = $el.querySelectorAll('.dy-month-table td a')[5];
  ...
  it('type:month', done => {
    ...
    setTimeout(_ => {
      expect(vm.picker.$el.querySelector('.dy-month-table').style.display).to.be.empty;
      expect(vm.picker.$el.querySelector('.dy-year-table').style.display).to.be.equal('none');
      vm.picker.$el.querySelector('.dy-month-table a.cell').click();
  ...
  it('type:year', done => {
    ...
    setTimeout(_ => {
      expect(vm.picker.$el.querySelector('.dy-year-table').style.display).to.empty;
      expect(vm.picker.$el.querySelector('.dy-month-table').style.display).to.be.equal('none');

      const leftBtn = vm.picker.$el.querySelector('.dy-icon-d-arrow-left');
      const rightBtn = vm.picker.$el.querySelector('.dy-icon-d-arrow-right');
      let count = 20;
      ...
      vm.picker.$el.querySelector('.dy-year-table a.cell').click();
      expect(vm.pickerVisible).to.false;
      done();
    }, DELAY);
  });
  ...
  describe('type:datetime', () => {
    ...
    it('create', done => {
      ...
      setTimeout(_ => {
        expect(vm.$refs.compo.picker.$el.querySelector('.dy-time-panel')).to.ok;
        done();
      }, DELAY);
    });
    ...
    it('both picker show correct formated value (extract date-format and time-format from format property)', done => {
      ...
      setTimeout(_ => {
        ...
        const dateInput = datePanel.$el.querySelector('.dy-date-picker__time-header > span:nth-child(1) input');
        const timeInput = datePanel.$el.querySelector('.dy-date-picker__time-header > span:nth-child(2) input');
        timeInput.focus();
        setTimeout(_ => {
    ...
    it('both picker show correct value', done => {
      ...
      setTimeout(_ => {
        ...
        setTimeout(_ => {
          ...
          const [hours, minutes, seconds] = timePanel.$el.querySelectorAll('.dy-time-spinner ul li.active');
    ...
    it('click now button', done => {
      ...
      setTimeout(_ => {
        ...
        vm.picker.$el.querySelector('.dy-picker-panel__link-btn').click();
        setTimeout(_ => {
    ...
    it('click timepicker', done => {
      ...
      setTimeout(_ => {
        const input = vm.picker.$el.querySelectorAll('.dy-date-picker__editor-wrap input')[1];
        triggerEvent(input, 'focus');

        setTimeout(_ => {
          const timePanel = vm.picker.$el.querySelector('.dy-time-panel');
    ...
    it('input date', done => {
      ...
      setTimeout(_ => {
        const input = vm.picker.$el.querySelector('.dy-date-picker__editor-wrap input');
    ...
    it('select time', done => {
      ...
      setTimeout(_ => {
        ...
        setTimeout(_ => {
          const button = vm.picker.$el.querySelector('.dy-time-panel .confirm');
          button.click();
      ...
      it('pick date, emits on confirm', done => {
        ...
        setTimeout(_ => {
          ...
          setTimeout(_ => {
            ...
            vm.$refs.compo.picker.$el.querySelector('.dy-picker-panel__footer .dy-button--default').click();
            setTimeout(_ => {
    ...
    describe('cancel time', () => {
      ...
      it('cancel to empty', done => {
        ...
        setTimeout(_ => {
          const timeInput = vm.$refs.compo.picker.$el.querySelector('.dy-date-picker__time-header > span:nth-child(2) input');
          timeInput.focus();
          setTimeout(_ => {
    ...
    it('now button: can not choose disabled date', done => {
      ...
      setTimeout(_ => {
        // click now button
        vm.$refs.compo.picker.$el.querySelector('.dy-picker-panel__footer .dy-button--text').click();
        setTimeout(_ => {
          expect(vm.value).to.equal('');
          done();
        }, DELAY);
    ...
    it('select time honors picked date', done => {
      ...
      setTimeout(_ => {
        ...
        vm.$refs.compo.picker.$el.querySelector('.dy-date-picker__header .dy-icon-arrow-right').click();
        setTimeout(_ => {
          vm.$refs.compo.picker.$el.querySelector('.dy-date-picker__header .dy-icon-d-arrow-right').click();
          setTimeout(_ => {
  ...
  describe('type:week', () => {
    ...
    it('click cell', done => {
      vm.picker.$el.querySelector('.dy-date-table__row .available').click();
      setTimeout(_ => {
    ...
    it('click year label', () => {
      vm.picker.$el.querySelector('.dy-date-picker__header-label').click();
      expect(vm.picker.currentView).to.equal('year');
    });

    it('click month label', () => {
      vm.picker.$el.querySelectorAll('.dy-date-picker__header-label')[1].click();
    ...
    it('select month', done => {
      ...
      setTimeout(_ => {
        vm.picker.$el.querySelector('.dy-month-table .cell').click();

        setTimeout(_ => {
          expect(vm.picker.$el.querySelector('.dy-date-table.is-week-mode').style.display).to.equal('');
          done();
        }, DELAY);
      }, DELAY);
    });
    ...
    it('highlight correctly', done => {
      ...
      setTimeout(() => {
        ...
        const numberOfHighlightRows = () => pickerEl.querySelectorAll('.dy-date-table__row.current').length;
        expect(numberOfHighlightRows()).to.equal(0);
        setTimeout(() => {
          pickerEl.querySelector('.dy-date-table__row ~ .dy-date-table__row td.available').click();
          setTimeout(() => {
            expect(vm.value).to.exist;
            input.blur();
            input.focus();
            setTimeout(() => {
              ...
              pickerEl.querySelector('.dy-icon-arrow-right').click();
              setTimeout(() => {
                expect(numberOfHighlightRows()).to.equal(0);
                // test: next year should not have highlight
                pickerEl.querySelector('.dy-icon-arrow-left').click(); // go back one month
                pickerEl.querySelector('.dy-icon-d-arrow-right').click();
                setTimeout(() => {
                  expect(numberOfHighlightRows()).to.equal(0);
                  done();
                }, DELAY);
  ...
  describe('type:dates', () => {
    ...
    it('click cell', done => {
      const td = vm.$refs.compo.picker.$el.querySelector('.dy-date-table__row .available');
      td.click();
      setTimeout(_ => {
  ...
  describe('type:daterange', () => {
    ...
    it('works', done => {
      ...
      setTimeout(_ => {
        const panels = rangePicker.picker.$el.querySelectorAll('.dy-date-range-picker__content');
    ...
    it('type:daterange unlink:true', done => {
      ...
      setTimeout(_ => {
        ...
        setTimeout(_ => {
          const panels = rangePicker.picker.$el.querySelectorAll('.dy-date-range-picker__content');
          const left = panels[0].querySelector('.dy-date-range-picker__header');
          const right = panels[1].querySelector('.is-right .dy-date-range-picker__header');
    ...
    it('unlink panels', done => {
      ...
      setTimeout(_ => {
        const panels = vm.picker.$el.querySelectorAll('.dy-date-range-picker__content');

        expect(Array.prototype.slice.call(panels)).to.length(2);

        panels[1].querySelector('.dy-icon-d-arrow-right').click();
        panels[1].querySelector('.dy-icon-arrow-right').click();
    ...
    describe('default value', () => {
      it('single', done => {
        ...
        setTimeout(_ => {
          ...
          defaultEls[0].click();
          setTimeout(_ => {
            $el.querySelector('.dy-date-table td.default + td').click();
            setTimeout(_ => {
              expect(vm.value).to.eql(expectValue);
              done();
            }, DELAY);
      ...
      it('array', done => {
        ...
        vm.$el.querySelector('input').focus();
        setTimeout(_ => {
          const defaultEls = vm.$refs.compo.picker.$el.querySelectorAll('.dy-date-table td.default');
  ...
  describe('type:datetimerange', () => {
    ...
    it('create', () => {
      expect(Array.prototype.slice.call(vm.picker.$el.querySelectorAll('.dy-time-panel'))).to.length(2);
    });

    it('select daterange', done => {
      const pickers = vm.picker.$el.querySelectorAll('.dy-date-range-picker__content');
    ...
    it('both picker show correct formated value (extract date-format and time-format from format property)', done => {
      ...
      setTimeout(_ => {
        ...
        compo.$el.click();
        setTimeout(_ => {
          const pickers = compo.picker.$el.querySelectorAll('.dy-date-range-picker__time-header .dy-date-range-picker__editors-wrap');
          const left = {
            dateInput: pickers[0].querySelector('.dy-date-range-picker__time-picker-wrap:nth-child(1) input'),
            timeInput: pickers[0].querySelector('.dy-date-range-picker__time-picker-wrap:nth-child(2) input')
          };
          const right = {
            dateInput: pickers[1].querySelector('.dy-date-range-picker__time-picker-wrap:nth-child(1) input'),
            timeInput: pickers[1].querySelector('.dy-date-range-picker__time-picker-wrap:nth-child(2) input')
          };
    ...
    it('prev/next month button', done => {
      const leftBtn = vm.picker.$el.querySelector('.is-left .dy-icon-arrow-left');
      const rightBtn = vm.picker.$el.querySelector('.is-right .dy-icon-arrow-right');
      const left = vm.picker.$el.querySelector('.is-left .dy-date-range-picker__header');
      const right = vm.picker.$el.querySelector('.is-right .dy-date-range-picker__header');
    ...
    it('prev/next year button', done => {
      const leftBtn = vm.picker.$el.querySelector('.is-left .dy-icon-d-arrow-left');
      const rightBtn = vm.picker.$el.querySelector('.is-right .dy-icon-d-arrow-right');
      const left = vm.picker.$el.querySelector('.is-left .dy-date-range-picker__header');
      const right = vm.picker.$el.querySelector('.is-right .dy-date-range-picker__header');
      const leftText = left.textContent.match(/\d+/g);
      const rightText = right.textContent.match(/\d+/g);
    ...
    it('input date', done => {
      const input = vm.picker.$el.querySelector('.dy-date-range-picker__editors-wrap input');
      const pickers = vm.picker.$el.querySelectorAll('.dy-date-range-picker__content');
      const leftCell = pickers[0].querySelector('td.available');
      const rightCell = pickers[1].querySelector('td.available');
    ...
    it('input date when minDate > maxDate', done => {
      const input = vm.picker.$el.querySelector('.dy-date-range-picker__editors-wrap input');
      const input2 = vm.picker.$el.querySelectorAll('.dy-date-range-picker__editors-wrap input')[2];
      const pickers = vm.picker.$el.querySelectorAll('.dy-date-range-picker__content');
      const leftCell = pickers[0].querySelector('td.available');
      const rightCell = pickers[1].querySelector('td.available');
    ...
    it('select time', done => {
      const input = vm.picker.$el.querySelectorAll('.dy-date-range-picker__editors-wrap input')[1];
      input.blur();
      input.focus();
      input.blur();

      setTimeout(_ => {
        const button = vm.picker.$el.querySelector('.dy-date-range-picker__time-picker-wrap .dy-time-panel .confirm');
        button.click();
  ...
  describe('type:monthrange', () => {
    ...
    describe('default value', () => {
      ...
      it('single', done => {
        ...
        setTimeout(_ => {
          const $el = vm.$refs.compo.picker.$el;
          const defaultEls = $el.querySelectorAll('.dy-month-table td.default');
          expect(defaultEls.length).to.equal(1);
          defaultEls[0].click();
          setTimeout(_ => {
            $el.querySelector('.dy-month-table td.default + td').click();
            setTimeout(_ => {
              expect(vm.value).to.eql(expectValue);
              done();
            }, DELAY);
      ...
      it('array', done => {
        ...
        vm.$el.querySelector('input').focus();
        setTimeout(_ => {
          const defaultEls = vm.$refs.compo.picker.$el.querySelectorAll('.dy-month-table td.default');
          expect(defaultEls.length).to.equal(2);
          defaultEls[0].click();
          setTimeout(_ => {
            defaultEls[1].click();
  ...
  it('picker-options:shortcuts', done => {
    ...
    setTimeout(_ => {
      const shortcut = vm.picker.$el.querySelector('.dy-picker-panel__shortcut');

      expect(shortcut.textContent).to.be.equal('今天');
      expect(vm.picker.$el.querySelector('.dy-picker-panel__sidebar')).to.be.ok;
  ...
  describe('picker-options:selectableRange', () => {
    ...
    it('selectableRange', done => {
      ...
      setTimeout(() => {
        const $el = vm.$refs.compo.picker.$el;
        const input1 = $el.querySelectorAll('.dy-date-picker__editor-wrap input')[1];
        input1.blur();
        input1.focus();
        setTimeout(_ => {
          const list = $el.querySelectorAll('.dy-time-spinner__list');
  ...
  describe('picker-options:className', () => {
    it('set custom class name', async() => {
      ...
      await wait();
      expect(
        (vm.$refs.compo.picker.$el.querySelector('.dy-date-table__row td').className)
