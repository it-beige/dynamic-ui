...
<template>
  <div class="page-container page-theme">
    ...
    <dy-dialog :visible.sync="copyDialogVisible">
      <dy-form :model="copyForm" ref="copyForm" :rules="copyFormRule">
        <dy-form-item label="<%= 3 >" prop="name">
          <dy-input v-model="copyForm.name"></dy-input>
        </dy-form-item>
      </dy-form>
      <div slot="footer" class="dialog-footer">
        <dy-button @click="closeCopyForm">{{getActionDisplayName('cancel')}}</dy-button>
        <dy-button type="primary" @click="copyToUser">{{getActionDisplayName('confirm')}}</dy-button>
      </div>
    </dy-dialog>
  </div>
</template>
