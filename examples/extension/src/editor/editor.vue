<template>
  <div class="ext-panel" :class="{moving : moving}" :style="{top: `${top}px`, left: `${left}px`}" ref="editor" >
    ...
    <div class="editor" :style="{height: `${height}px`}" v-show="showSidebar">
      <dy-tabs v-model="activeTab" @tab-click="onTabClick">
        <dy-tab-pane label="Config" name="config">
          <theme-configurator
        ...
        <dy-tab-pane label="Gallery" name="gallery">
          <gallery 
            ref='gallery'
            :height="height"
            :width="width - 7"
...
<style>
.ext-panel  .editor .dy-tabs__content, .ext-panel  .editor .dy-tabs--top, .ext-panel  .editor .dy-tab-pane {
  height: 100%;
}
.ext-panel .dy-tabs__nav-scroll >div {
