<template>
  <table
    ...
    <tbody>
    <tr>
      <th v-if="showWeekNumber">{{ t('dy.datepicker.week') }}</th>
      <th v-for="(week, key) in WEEKS" :key="key">{{ t('dy.datepicker.weeks.' + week) }}</th>
    </tr>
    <tr
      class="dy-date-table__row"
...
<script>
  import { getFirstDayOfMonth, getDayCountOfMonth, getWeekNumber, getStartDateOfMonth, prevDate, nextDate, isDate, clearTime as _clearTime} from 'dynamic-ui/src/utils/date-util';
  import Locale from 'dynamic-ui/src/mixins/locale';
  import { arrayFindIndex, arrayFind, coerceTruthyValueToArray } from 'dynamic-ui/src/utils/util';
