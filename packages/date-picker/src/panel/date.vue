<template>
  <transition name="dy-zoom-in-top" @after-enter="handleEnter" @after-leave="handleLeave">
    <div
      v-show="visible"
      class="dy-picker-panel dy-date-picker dy-popper"
      :class="[{
      ...
      <div class="dy-picker-panel__body-wrapper">
        ...
        <div class="dy-picker-panel__body">
          <div class="dy-date-picker__time-header" v-if="showTime">
            <span class="dy-date-picker__editor-wrap">
              <dy-input
                :placeholder="t('dy.datepicker.selectDate')"
                :value="visibleDate"
                size="small"
                @input="val => userInputDate = val"
                @change="handleVisibleDateChange" />
            </span>
            <span class="dy-date-picker__editor-wrap" v-clickoutside="handleTimePickClose">
              <dy-input
                ref="input"
                @focus="timePickerVisible = true"
                :placeholder="t('dy.datepicker.selectTime')"
                :value="visibleTime"
                size="small"
          ...
          <div
            class="dy-date-picker__header"
            :class="{ 'dy-date-picker__header--bordered': currentView === 'year' || currentView === 'month' }"
            v-show="currentView !== 'time'">
            <button
              type="button"
              @click="prevYear"
              :aria-label="t(`el.datepicker.prevYear`)"
              class="dy-picker-panel__icon-btn dy-date-picker__prev-btn dy-icon-d-arrow-left">
            </button>
            <button
              type="button"
              @click="prevMonth"
              ...
              class="dy-picker-panel__icon-btn dy-date-picker__prev-btn dy-icon-arrow-left">
            </button>
            <span
              @click="showYearPicker"
              role="button"
              class="dy-date-picker__header-label">{{ yearLabel }}</span>
            <span
              @click="showMonthPicker"
              v-show="currentView === 'date'"
              role="button"
              class="dy-date-picker__header-label"
              :class="{ active: currentView === 'month' }">{{t(`el.datepicker.month${ month + 1 }`)}}</span>
            <button
              type="button"
              @click="nextYear"
              :aria-label="t(`el.datepicker.nextYear`)"
              class="dy-picker-panel__icon-btn dy-date-picker__next-btn dy-icon-d-arrow-right">
            </button>
            <button
              type="button"
              @click="nextMonth"
              ...
              class="dy-picker-panel__icon-btn dy-date-picker__next-btn dy-icon-arrow-right">
            </button>
          </div>

          <div class="dy-picker-panel__content">
            <date-table
              v-show="currentView === 'date'"
              @pick="handleDatePick"
...
<script type="text/babel">
  ...
  export default {
    ...
    components: {
      TimePicker, YearTable, MonthTable, DateTable, DyInput, DyButton
    },
    ...
    computed: {
      ...
      yearLabel() {
        const yearTranslation = this.t('dy.datepicker.year');
        if (this.currentView === 'year') {
