<template>
  <transition name="dy-zoom-in-top" @after-leave="$emit('dodestroy')">
    <div
      v-show="visible"
      class="dy-picker-panel dy-date-range-picker dy-popper"
      :class="[{
        'has-sidebar': $slots.sidebar || shortcuts,
        'has-time': showTime
      }, popperClass]">
      <div class="dy-picker-panel__body-wrapper">
        <slot name="sidebar" class="dy-picker-panel__sidebar"></slot>
        <div class="dy-picker-panel__sidebar" v-if="shortcuts">
          <button
            type="button"
            class="dy-picker-panel__shortcut"
            v-for="(shortcut, key) in shortcuts"
            :key="key"
            @click="handleShortcutClick(shortcut)">{{shortcut.text}}</button>
        </div>
        <div class="dy-picker-panel__body">
          <div class="dy-date-range-picker__time-header" v-if="showTime">
            <span class="dy-date-range-picker__editors-wrap">
              <span class="dy-date-range-picker__time-picker-wrap">
                <dy-input
                  size="small"
                  :disabled="rangeState.selecting"
                  ref="minInput"
                  :placeholder="t('dy.datepicker.startDate')"
                  class="dy-date-range-picker__editor"
                  :value="minVisibleDate"
                  @input="val => handleDateInput(val, 'min')"
                  @change="val => handleDateChange(val, 'min')" />
              </span>
              <span class="dy-date-range-picker__time-picker-wrap" v-clickoutside="handleMinTimeClose">
                <dy-input
                  size="small"
                  class="dy-date-range-picker__editor"
                  :disabled="rangeState.selecting"
                  :placeholder="t('dy.datepicker.startTime')"
            ...
            <span class="dy-icon-arrow-right"></span>
            <span class="dy-date-range-picker__editors-wrap is-right">
              <span class="dy-date-range-picker__time-picker-wrap">
                <dy-input
                  size="small"
                  class="dy-date-range-picker__editor"
                  :disabled="rangeState.selecting"
                  :placeholder="t('dy.datepicker.endDate')"
                  :value="maxVisibleDate"
                  :readonly="!minDate"
                  @input="val => handleDateInput(val, 'max')"
                  @change="val => handleDateChange(val, 'max')" />
              </span>
              <span class="dy-date-range-picker__time-picker-wrap" v-clickoutside="handleMaxTimeClose">
                <dy-input
                  size="small"
                  class="dy-date-range-picker__editor"
                  :disabled="rangeState.selecting"
                  :placeholder="t('dy.datepicker.endTime')"
          ...
          <div class="dy-picker-panel__content dy-date-range-picker__content is-left">
            <div class="dy-date-range-picker__header">
              <button
                type="button"
                @click="leftPrevYear"
                class="dy-picker-panel__icon-btn dy-icon-d-arrow-left"></button>
              <button
                type="button"
                @click="leftPrevMonth"
                class="dy-picker-panel__icon-btn dy-icon-arrow-left"></button>
              <button
                type="button"
                @click="leftNextYear"
                v-if="unlinkPanels"
                ...
                class="dy-picker-panel__icon-btn dy-icon-d-arrow-right"></button>
              <button
                type="button"
                @click="leftNextMonth"
                v-if="unlinkPanels"
                ...
                class="dy-picker-panel__icon-btn dy-icon-arrow-right"></button>
              <div>{{ leftLabel }}</div>
            </div>
            <date-table
          ...
          <div class="dy-picker-panel__content dy-date-range-picker__content is-right">
            <div class="dy-date-range-picker__header">
              <button
                type="button"
                @click="rightPrevYear"
      ...
      <div class="dy-picker-panel__footer" v-if="showTime">
        <dy-button
          size="mini"
          type="text"
          class="dy-picker-panel__link-btn"
          @click="handleClear">
          {{ t('dy.datepicker.clear') }}
        </dy-button>
        <dy-button
          plain
          size="mini"
...
<script type="text/babel">
  import {
    ...
    extractDateFormat,
    extractTimeFormat
  } from 'dynamic-ui/src/utils/date-util';
  ...
  export default {
    ...
    computed: {
      ...
      leftLabel() {
        return this.leftDate.getFullYear() + ' ' + this.t('dy.datepicker.year') + ' ' + this.t(`el.datepicker.month${ this.leftDate.getMonth() + 1 }`);
      },

      rightLabel() {
        return this.rightDate.getFullYear() + ' ' + this.t('dy.datepicker.year') + ' ' + this.t(`el.datepicker.month${ this.rightDate.getMonth() + 1 }`);
      },
    ...
    components: { TimePicker, DateTable, DyInput, DyButton }
  };
</script>
