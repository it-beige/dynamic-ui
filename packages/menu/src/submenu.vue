<script>
  ...
  export default {
    ...
    computed: {
      ...
      menuTransitionName() {
        return this.rootMenu.collapse ? 'dy-zoom-in-left' : 'dy-zoom-in-top';
      },
      opened() {
      ...
      isFirstLevel() {
        ...
        while (parent && parent !== this.rootMenu) {
          if (['DySubmenu', 'DyMenuItemGroup'].indexOf(parent.$options.componentName) > -1) {
            isFirstLevel = false;
            break;
          } else {
    ...
    methods: {
      ...
      handleClick() {
        ...
        this.dispatch('DyMenu', 'submenu-click', this);
      },
      handleMouseenter(event, showTimeout = this.showTimeout) {
        ...
        this.dispatch('DySubmenu', 'mouse-enter-child');
      ...
      handleMouseleave(deepDispatch = false) {
        ...
        this.dispatch('DySubmenu', 'mouse-leave-child');
        clearTimeout(this.timeout);
        this.timeout = setTimeout(() => {
        ...
        if (this.appendToBody && deepDispatch) {
          if (this.$parent.$options.name === 'DySubmenu') {
    ...
    render(h) {
      ...
      const popupMenu = (
        <transition name={menuTransitionName}>
          <div
            ref="menu"
            v-show={opened}
            class={[`dy-menu--${mode}`, popperClass]}
            ...
            <ul
              role="menu"
              class={['dy-menu dy-menu--popup', `dy-menu--popup-${currentPlacement}`]}
      ...
      ) ? 'dy-icon-arrow-down' : 'dy-icon-arrow-right';
      ...
      return (
        ...
        >
          ...
          >
            {$slots.title}
            <i class={[ 'dy-submenu__icon-arrow', submenuTitleIcon ]}></i>
          </div>
