    ...
    >
      <slot :file="file">
        ...
        <a class="dy-upload-list__item-name" @click="handleClick(file)">
          ...
          <div class="dy-flex-column dy-info" v-if="listType === 'card'">
            <label class="file-name"> {{file.name}}</label>
            <span class="file-size">{{file.size}}KB</span>
          </div>
          <label v-else class="file-name"> {{file.name}}</label>
         
        </a>
        <label class="dy-upload-list__item-status-label">
          <i :class="{
            'dy-icon-upload-success': true,
            'dy-icon-circle-check': ['text', 'card'].includes(listType),
            'dy-icon-check': ['picture-card', 'picture', 'card'].indexOf(listType) > -1
          }"></i>
        </label>
        <i :class="listType === 'text' ? 'dy-icon-download': 'dy-icon3-download'" v-if="['text', 'card'].includes(listType)" @click="$emit('download', file)"></i>
