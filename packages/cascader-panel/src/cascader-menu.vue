...
export default {
  ...
  methods: {
    ...
    renderEmptyText(h) {
      return (
        <div class="dy-cascader-menu__empty-text">{ this.t('dy.cascader.noData') }</div>
      );
    },
    renderNodeList(h) {
      const { menuId } = this;
      ...
      return [
        ...nodes,
        isHoverMenu ? <svg ref='hoverZone' class='dy-cascader-menu__hover-zone'></svg> : null
      ];
    }
  },
  ...
  render(h) {
    ...
    return (
      <dy-scrollbar
        ...
        role="menu"
        id={ menuId }
        class="dy-cascader-menu"
        wrap-class="dy-cascader-menu__wrap"
        view-class={{
          'dy-cascader-menu__list': true,
          'is-empty': isEmpty
        }}
        { ...events }>
