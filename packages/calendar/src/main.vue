<template>
  <div class="dy-calendar">
    <div class="dy-calendar__header">
      <div class="dy-calendar__title">
        {{ i18nDate }}
      </div>
      <div
        class="dy-calendar__button-group"
        ...
        <dy-button-group>
          <dy-button
            ...
            {{ t('dy.datepicker.prevMonth') }}
          </dy-button>
          <dy-button
            type="plain"
            size="mini"
            @click="selectDate('today')">
            {{ t('dy.datepicker.today') }}
          </dy-button>
          <dy-button
            type="plain"
            size="mini"
            @click="selectDate('next-month')">
            {{ t('dy.datepicker.nextMonth') }}
          </dy-button>
        </dy-button-group>
      </div>
    </div>
    <div
      class="dy-calendar__body"
      v-if="validatedRange.length === 0"
      key="no-range">
      <date-table
        :date="date"
...
import fecha from 'dynamic-ui/src/utils/date';
import DyButton from 'dynamic-ui/packages/button';
import DyButtonGroup from 'dynamic-ui/packages/button-group';
import DateTable from './date-table';
import { validateRangeInOneMonth } from 'dynamic-ui/src/utils/date-util';
...
export default {
  ...
  methods: {
    ...
    rangeValidator(date, isStart) {
      ...
      if (date.getDay() !== expected) {
        console.warn('[DynamicCalendar]', message, 'Invalid range will be ignored.');
        return false;
      }
      return true;
    }
  ...
  computed: {
    ...
    i18nDate() {
      ...
      return `${year} ${this.t('dy.datepicker.year')} ${this.t('dy.datepicker.month' + month)}`;
    },
    ...
    validatedRange() {
      ...
      if (range.length === 2) {
        ...
        if (start > end) {
          console.warn('[DynamicCalendar]end time should be greater than start time');
          return [];
        }
        ...
        if (!validateRangeInOneMonth(startDay, end)) {
          console.warn('[DynamicCalendar]start time and end time interval must not exceed two months');
          return [];
        }
        // 第一个月的时间范围
        data.push([
