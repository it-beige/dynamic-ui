<template>
  <label
    class="dy-radio"
    :class="[
      border && radioSize ? 'dy-radio--' + radioSize : '',
  ...
  >
    <span class="dy-radio__input"
      :class="{
        'is-disabled': isDisabled,
        'is-checked': model === label
      }"
    >
      <span class="dy-radio__inner"></span>
      <input
        ref="radio"
        class="dy-radio__original"
        :value="label"
        type="radio"
        aria-hidden="true"
        v-model="model"
    ...
    <span class="dy-radio__label" @keydown.stop>
      <slot></slot>
