<script>
  ...
  export default {
    ...
    watch: {
      visible(val) {
        this.broadcast('DyDropdownMenu', 'visible', val);
        this.$emit('visible-change', val);
      },
      focusing(val) {
        const selfDefine = this.$el.querySelector('.dy-dropdown-selfdefine');
        if (selfDefine) { // 自定义
          if (val) {
    ...
    methods: {
      ...
      initAria() {
        ...
        if (!this.splitButton) { // 自定义
          ...
          this.triggerElm.setAttribute('class', (this.triggerElm.getAttribute('class') || '') + ' dy-dropdown-selfdefine'); // 控制
        }
      },
      initEvent() {
    ...
    render(h) {
      ...
      let triggerElm = null;
      if (splitButton) {
        triggerElm = <dy-button-group>
          <dy-button type={type} size={dropdownSize} nativeOn-click={handleMainButtonClick} disabled={disabled}>
            {this.$slots.default}
          </dy-button>
          <dy-button ref="trigger" type={type} size={dropdownSize} class="dy-dropdown__caret-button" disabled={disabled}>
            <i class="dy-dropdown__icon dy-icon-arrow-down"></i>
          </dy-button>
        </dy-button-group>;
      } else {
      ...
      return (
        <div class="dy-dropdown" v-clickoutside={hide} aria-disabled={disabled}>
          {triggerElm}
          {menuElm}
        </div>
      );
