<template>
  <div
    class="dy-slider"
    :class="{ 'is-vertical': vertical, 'dy-slider--with-input': showInput }"
    role="slider"
    :aria-valuemin="min"
    :aria-valuemax="max"
  ...
  >
    ...
    <div
      class="dy-slider__runway"
      :class="{ 'show-input': showInput, 'disabled': sliderDisabled }"
      :style="runwayStyle"
      @click="onSliderClick"
      ref="slider">
      <div
        class="dy-slider__bar"
        :style="barStyle">
      </div>
      <slider-button
        :vertical="vertical"
      ...
      <template v-if="markList.length > 0">
        <div>
          <div
            ...
            class="dy-slider__stop dy-slider__marks-stop"
            :key="key">
          </div>
        </div>
        <div class="dy-slider__marks">
          <slider-marker
...
<script type="text/babel">
  import DyInputNumber from 'dynamic-ui/packages/input-number';
    ...
    methods: {
      ...
      setValues() {
        if (this.min > this.max) {
          console.error('[Dynamic Error][Slider]min should not be greater than max.');
          return;
        }
    ...
    computed: {
      stops() {
        ...
        if (this.step === 0) {
          ...
          console.warn('[Dynamic Warn][Slider]step should not be 0.');
          return [];
        }
