<template>
  <div
    ...
    <dy-input
      ...
      <template slot="suffix">
        <i
          ...
          key="clear"
          class="dy-input__icon dy-icon-circle-close"
    ...
    <div v-if="multiple" class="dy-cascader__tags">
      <dy-tag
      ...
      <input
        ...
        type="text"
        class="dy-cascader__search-input"
    ...
    <transition name="dy-zoom-in-top" @after-leave="handleDropdownLeave">
      <div
        v-show="dropDownVisible"
        ref="popper"
        :class="['dy-popper', 'dy-cascader__dropdown', popperClass]">
        <dy-cascader-panel
          ref="panel"
          v-show="!filtering"
          ...
          @close="toggleDropDownVisible(false)"></dy-cascader-panel>
        <dy-scrollbar
          ref="suggestionPanel"
          v-if="filterable"
          v-show="filtering"
          tag="ul"
          class="dy-cascader__suggestion-panel"
          view-class="dy-cascader__suggestion-list"
          ...
          <template v-if="suggestions.length">
            <li
              ...
              :key="item.uid"
              :class="[
                'dy-cascader__suggestion-item',
                item.checked && 'is-checked'
              ]"
              :tabindex="-1"
              @click="handleSuggestionClick(index)">
              <span>{{ item.text }}</span>
              <i v-if="item.checked" class="dy-icon-check"></i>
            </li>
          </template>
          <slot v-else name="empty">
            <li class="dy-cascader__empty-text">{{ t('dy.cascader.noMatch') }}</li>
          </slot>
        </dy-scrollbar>
      </div>
    </transition>
...
import DyCascaderPanel from 'dynamic-ui/packages/cascader-panel';
import AriaUtils from 'dynamic-ui/src/utils/aria-utils';
import { t } from 'dynamic-ui/src/locale';
import { isEqual, isEmpty, kebabCase } from 'dynamic-ui/src/utils/util';
import { isUndefined, isFunction } from 'dynamic-ui/src/utils/types';
import { isDef } from 'dynamic-ui/src/utils/shared';
...
export default {
  ...
  props: {
    ...
    placeholder: {
      type: String,
      default: () => t('dy.cascader.placeholder')
    },
    disabled: Boolean,
    clearable: Boolean,
    filterable: Boolean,
  ...
  computed: {
    parseModel: {
      get({multiple, presentText, inputValue}) {
        return multiple ? presentText : inputValue;
      },
      set(v) {
        const prop = this.multiple ? 'presentText' : 'inputValue';
        this[prop] = v;
      }
    },
    realSize() {
  ...
  methods: {
    ...
    focusFirstNode() {
      this.$nextTick(() => {
        ...
        if (filtering && suggestionPanel) {
          firstNode = suggestionPanel.$el.querySelector('.dy-cascader__suggestion-item');
        } else {
          const firstMenu = popper.querySelector('.dy-cascader-menu');
          firstNode = firstMenu.querySelector('.dy-cascader-node[tabindex="-1"]');
        }
    ...
    updateStyle() {
      ...
      const inputInner = $el.querySelector('.dy-input__inner');

      if (!inputInner) return;

      const tags = $el.querySelector('.dy-cascader__tags');
      let suggestionPanelEl = null;

      if (suggestionPanel && (suggestionPanelEl = suggestionPanel.$el)) {
        const suggestionList = suggestionPanelEl.querySelector('.dy-cascader__suggestion-list');
