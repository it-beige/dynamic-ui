import { arrayFindIndex } from 'dynamic-ui/src/utils/util';
import { getCell, getColumnByCell, getRowIdentity } from './util';
import { getStyle, hasClass, removeClass, addClass } from 'dynamic-ui/src/utils/dom';
import DyCheckbox from 'dynamic-ui/packages/checkbox';
import DyTooltip from 'dynamic-ui/packages/tooltip';
...
export default {
    ...
    return (
      <table
        ...
        <tbody>
          ...
          <dy-tooltip effect={this.table.tooltipEffect} placement="top" ref="tooltip" content={this.tooltipContent}></dy-tooltip>
        </tbody>
      </table>
    );
  },
  ...
  watch: {
    ...
    'store.states.hoverRow'(newVal, oldVal) {
      ...
      raf(() => {
        const rows = this.$el.querySelectorAll('.dy-table__row');
  ...
  methods: {
    ...
    getRowClass(row, rowIndex) {
      ...
      if (this.stripe && rowIndex % 2 === 1) {
        classes.push('dy-table__row--striped');
      }
    ...
    handleCellMouseEnter(event, row) {
      ...
      if (!(hasClass(cellChild, 'dy-tooltip') && cellChild.childNodes.length)) {
        return;
      }
    ...
    rowRender(row, $index, treeRowData) {
      ...
      let display = true;
      if (treeRowData) {
        rowClasses.push('dy-table__row--levdy-' + treeRowData.level);
    ...
    wrappedRowRender(row, $index) {
      ...
      if (this.hasExpandColumn && isRowExpanded(row)) {
        ...
        if (!renderExpanded) {
          console.error('[Dynamic Error]renderExpanded is required.');
          return tr;
        }
        // 使用二维数组，避免修改 $index
        return [[
          ...
          <tr key={'expanded-row__' + tr.key}>
            <td colspan={ this.columnsCount } class="dy-table__cell dy-table__expanded-cell">
