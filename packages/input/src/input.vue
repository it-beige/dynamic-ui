  ...
  >
    <template v-if="type !== 'textarea'">
      ...
      <span class="dy-input__prefix" v-if="$slots.prefix || prefixIcon">
        ...
        <i class="dy-input__icon" v-if="prefixIcon" :class="prefixIcon"></i>
      </span>
      <!-- 后置内容 -->
      <span class="dy-input__suffix" v-if="getSuffixVisible()">
        <span class="dy-input__suffix-inner">
          <template v-if="!showClear || !showPwdVisible || !isWordLimitVisible">
            <slot name="suffix"></slot>
            <i class="dy-input__icon" v-if="suffixIcon" :class="suffixIcon"></i>
          </template>
          <i
            v-if="showClear"
    ...
    autoComplete: {
      ...
      validator(val) {
        process.env.NODE_ENV !== 'production' &&
          console.warn(
            "[Dynamic Warn][Input]'auto-complete' property will be deprecated in next major version. please use 'autocomplete' instead.",
          );
        return true;
      }
    },
  ...
  methods: {
    ...
    calcIconOffset(place) {
      let elList = [].slice.call(
        this.$el.querySelectorAll(`.dy-input__${place}`) || [],
      );
      ...
      if (this.$slots[pendant]) {
        el.style.transform = `translateX(${place === 'suffix' ? '-' : ''}${
          this.$el.querySelector(`.dy-input-group__${pendant}`).offsetWidth
        }px)`;
      } else {
